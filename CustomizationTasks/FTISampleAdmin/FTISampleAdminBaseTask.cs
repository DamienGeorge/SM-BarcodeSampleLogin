...
namespace Customization.Tasks
{
    public class FTISampleAdminBaseTask
    {
        ...
        public FTISampleAdminBaseTask(FormSampleAdmin MainForm, StandardLibrary library)
        {
        ...
        private static void UpdateJobDisplayText(JobHeader jobNode, SimpleTreeListNodeProxy node, UnboundGridValueChangedEventArgs e = null)
        {
            if (node != null)
            {
        ...
        public void UpdateTreeList(SimpleTreeListNodeProxy m_RootNode)
        {
            ...
            if (m_RootNode.DisplayText == "Jobs")
            {
                foreach (var node in m_RootNode.Nodes)
                {
                    ...
                    {
                        UpdateJobDisplayText(jobHeader, m_RootNode);

                        IterateChildSamples(jobHeader.Samples);
                    }
            ...
            else if (m_RootNode.DisplayText == "Samples")
            {
                foreach (var node in m_RootNode.Nodes)
                {
                    if (node.Data is Sample sample)
                    {
                        IterateChildSampleNode(sample, node);
                    }
                }
            }
        }

        private void IterateChildSamples(IEntityCollection samples)
        {
            foreach (Sample sample in samples)
            {
                var sampleNode = _treeList.FindNodeByData(sample);
                IterateChildSampleNode(sample, sampleNode);
            }
        }

        private void IterateChildSampleNode(Sample sample, SimpleTreeListNodeProxy sampleNode)
        {
            UpdateSampleDisplayTextByNode(sampleNode);

            if (sample.ChildSamples.Count > 0)
            {
                IterateChildSamples(sample.ChildSamples);
            }
        }
    }
}