...
{
    [SampleManagerTask("USampleReceiptTask")]
    public class USampleReceiptTask : DefaultFormTask
    {
        ...
        private void AddRow(string scanText)
        {
            try
            {
                //TODO - Catch FatalWrofklow Error
                ...
                foreach (var entity in newEntities)
                {
                    ...
                    for (int i = m_UnboundGrid.FixedColumns; i < m_UnboundGrid.Columns.Count; i++)
                    {
                        ...
                        if (templateProperty.IsMandatory)
                        {
                            ...
                            //if (entity is Test)
                            //{
                            //	// Make the cell appear yellow unless assign is unchecked
                            //	// and add the cell to the lookup table

                            //	if (!m_MandatoryLookup.ContainsKey(name))
                            //	{
                            //		// This is a new entity, so add its row/column references
                            //		m_MandatoryLookup[name] = new List<MandatoryCellHelper>
                            //			{ new MandatoryCellHelper(name, newRow, column) };
                            //	}
                            //	else
                            //	{
                            //		// Already know about this entity so either replace or add its row/column references
                            //		// ReSharper disable once ForCanBeConvertedToForeach
                            //		for (var cell = 0; cell < m_MandatoryLookup.Count; cell++)
                            //		{
                            //			if (!m_MandatoryLookup.TryGetValue(name, out var list)) continue;

                            //			var index = GetIndexOfColumn(list, column);

                            //			if (index != -1)
                            //			{
                            //				// Already exists, so replace the row/column references
                            //				m_MandatoryLookup[name][index] = new MandatoryCellHelper(name, newRow, column);
                            //			}
                            //			else
                            //			{
                            //				// Doesn't exist, so add the row/column references
                            //				m_MandatoryLookup[name].Add(new MandatoryCellHelper(name, newRow, column));
                            //			}
                            //		}
                            //	}
                            //}
